...
function detectPlatform(chunk: string): Platform {
...
// or on a separator line (---, ===, ***), or on lines like "2." / "2)" numbering.
function splitListings(source: string): string[] {
  const normalized = source.replace(/\r/g, "").replace(/^\s*(?:[-=*_]{3,})\s*$/gm, "\n");
...
let nextId = 1;
function parseChunk(chunk: string): Entry {
  const firstLine = (chunk.split("\n").find(Boolean)?.trim() || "").replace(/^\d+[.)]\s+/, "");
  const company = /at\s+([^\n,.]+)/i.exec(chunk)?.[1]?.trim() || "";
  return {
    id: nextId++,
    company,
    role: firstLine && company ? firstLine.replace(new RegExp(`\\s+at\\s+${company.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*$`, "i"), "").slice(0, 80) : firstLine.slice(0, 80),
    date: detectDate(chunk),
